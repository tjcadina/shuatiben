...
assert.strictEqual(ctx.__run('session.index'), 1, 'left swipe goes to next question');

// 向右滑 -> 上一题
assert.strictEqual(ctx.__run('handlePracticeSwipe(90, 4)'), true, 'right swipe handled');
assert.strictEqual(ctx.__run('session.index'), 0, 'right swipe goes to previous question');
...
ctx.__run('handlePracticeSwipe(-90, 4)');
assert.strictEqual(ctx.__run('currentView'), 'report', 'left swipe after finishing last question opens report');
